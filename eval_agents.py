...
from slimevolleygym.neat import makeSlimeNeatPolicy
...
def rollout(env, policy0, policy1, render_mode=False):
  ...
  # Function to capture and store frames from the environment
  def capture_frame(env):      
    if len(frames) < 1000:
      frame = env.render(mode='rgb_array')
      # Convert to the desired format (e.g., RGB to BGR for OpenCV)
  ...
  # gif_filename = 'slimevolley.gif'
  # imageio.mimsave(gif_filename, frames, duration=20)
...
if __name__=="__main__":
  ...
  PATH = {
    ...
    "random": None,
    "neat": "neat_bl_p500_multiacts0.5_centerforehead_k10/neat_winner.pckl",
  }
  # neat_bl_p250_multiacts0.5_forehead_gen1000: neat scored -1.852 ± 1.652 vs baseline over 1000 trials.
  # neat_bl_p500_multiacts0.3_forehead_gen60: neat scored -1.854 ± 1.745 vs baseline over 1000 trials.
  # neat_bl_p500_multiacts0.3_forehead: neat scored -1.981 ± 1.64 vs baseline over 1000 trials.
  # neat_bl_p500_multiacts0.2_dist0.2only: neat scored -1.989 ± 1.532 vs baseline over 1000 trials.
  # neat_bl_p500_multiacts_dist0.2only: neat scored -2.121 ± 1.588 vs baseline over 1000 trials.
  ...
  MODEL = {
    ...
    "random": RandomPolicy,
    "neat": makeSlimeNeatPolicy,
  }

  parser = argparse.ArgumentParser(description='Evaluate pre-trained agents against each other.')
  parser.add_argument('--left', help='choice of (baseline, ppo, cma, ga, random, neat)', type=str, default="baseline")
  parser.add_argument('--leftpath', help='path to left model (leave blank for zoo)', type=str, default="")
  parser.add_argument('--right', help='choice of (baseline, ppo, cma, ga, random, neat)', type=str, default="ga")
