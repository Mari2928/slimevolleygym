...
def singleagent_rollout(env, policy_right, render_mode=False):
  """
  play against baseline in modified gym-style loop.
  ...
  while not done:
    ...
    obs_right, reward, done, info = env.step(action_right)

    ball = [info['state'][4], info['state'][5]]
    current = [info['state'][0], info['state'][1]]
    distance = math.sqrt((ball[0] - current[0])**2 + (ball[1] - current[1])**2)

    if distance < 0.2:   # ball hit
      fitness += 20
    if reward < 0:       # ball dropped
      fitness -= distance

    total_reward += reward
    t += 1
    #print(done, "reward", reward, "distance", distance, "fitness", fitness, "t", t)

    if render_mode:
      env.render()

  return total_reward, t, fitness
...
def multiagent_rollout(env, policy_right, policy_left, render_mode=False):
  ...
  while not done:
    ...
    # if distance < 0.30 and current[1] > 0.4:  # OPTION 1: jump and hit the ball 
    # if distance < 0.30 and current[1] > 0.4 and \
    #     (ball[0] < current[0]) and (current[1] < ball[1]):  # OPTION2: ball is on forehead while jumping
    if distance < 0.30 and current[1] > 0.4 and \
        (ball[0] < current[0]-0.05) and (current[1] < ball[1]):  # OPTION3: center of forehead while jumping
      fitness += 20
    if reward < 0:       # ball dropped
      fitness -= distance
